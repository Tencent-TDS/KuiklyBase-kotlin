...
class Converter(
    ...
    fun hprofObjectReferenceId(id: Id): HProfId {
        var hid = HProfId.NULL
        try {
            hid = hprofObjectId(id)
        } catch (_:IllegalArgumentException) {
        }
        return kotlinToJavaHprofIdMutableMap.getOrDefault(hid, hid)
    }
    // endregion
    ...
    fun buildProfile(): HProfProfile {
        //@Tencent begin
        // 获取 长度为 0 的字符串，防止 MAT 解析报错
        hprofProfileRecords.forEach {
            if (it is StringConstant && it.string.isEmpty()) {
                it.string = "EMPTY"
            }
        }
        //@Tencent end
