...
class CachedLibraries(
        ...
        private val targetFingerprint: String,
) {
    ...
    private inline fun <T> File.runIfMetadataMatches(block: () -> T): T? {
        val metadata = cacheMetadataFile ?: run {
            configuration.report(CompilerMessageSeverity.WARNING, "Cache at $absolutePath is broken: no metadata file")
            return null
        }
        val fits = metadata.checkMetadataFits(target, compilerFingerprint, targetFingerprint)
        when (fits) {
            is MetadataCheckResult.Ok -> return block()
