...
int polyHash_arm(int length, uint8_t const* str) {
...
struct NeonTraits {
    ...
    ALWAYS_INLINE static void u16Load(U8VecType const* x, VecType& low, VecType& high) {
        auto vec = vmovl_u8(*x);
        low = vmovl_u16(vget_low_u16(vec));
        high = vmovl_u16(vget_high_u16(vec));
    }
...
#include "polyhash_u8/attributeSensitiveFunctions.inc"

    static int polyHashUnalignedUnrollUpTo16(int n, uint8_t const* str) {
    ...
    static int polyHashUnalignedUnrollUpTo32(int n, uint8_t const* str) {
        Vec128Type res = initVec128();
