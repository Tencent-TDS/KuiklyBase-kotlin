...
fun <T> T.hashCode(): Int {
    return (this as Any?).hashCode()
}

fun <T> T.toString(): String {
    return (this as Any?).toString()
}
...
internal fun <T> Array<T>.copyOfUninitializedElements(newSize: Int): Array<T> {
    require(newSize >= 0) { "capacity must be non-negative." }
    @Suppress("UNCHECKED_CAST")
...
internal fun <E> Array<E>.resetRange(fromIndex: Int, toIndex: Int) {
    @Suppress("UNCHECKED_CAST")
    (this as Array<Any?>).fill(null, fromIndex, toIndex)
}

class HashMap<K, V> private constructor(
    // keys in insert order
    private var keysArray: Array<K>,
    // values in insert order, allocated only when ly used, always null in pure HashSet
     ...
     */
    public constructor() : this(INITIAL_CAPACITY)
     ...
     */
    public constructor(original: Map<out K, V>) : this(original.size) {
        putAll(original)
    }
     ...
     */
    public constructor(initialCapacity: Int, loadFactor: Float) : this(initialCapacity) {
    ...
    private fun ensureCapacity(minCapacity: Int) {
        if (minCapacity < 0) throw Exception("OutOfMemory")    // overflow
...
//        require(newHashSize > hashSize) { "Rehash can only be executed with a grown hash array" }
    ...
    internal class EntriesItr<K, V>(map: HashMap<K, V>) : Itr<K, V>(map),
        ...
        fun nextAppendString(sb: StringBuilder) {
            ...
            if (key === map) sb.append("(this Map)") else sb.append(key.toString())
            sb.append('=')
            val value = map.valuesArray!![lastIndex]
            if (value === map) sb.append("(this Map)") else sb.append(value.toString())
            initNext()
        }
    }
...
) : MutableSet<E>, KonanSet<E>, AbstractMutableSet<E>() {
...
public typealias LinkedHashMap<K, V> = HashMap<K, V>
